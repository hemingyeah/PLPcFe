<template>
  <el-dialog :visible.sync="visible" width="600px" >
    ...
    <div slot="footer">
      ...
      <el-button type="primary" :disabled="pending" :loading="pending" @click="exportData" size="small">{{pending ? '正在导出' : '导出'}}</el-button>
    </div>
...
export default {
  ...
  methods: {
    ...
    async exportData(){
      ...
      if (this.method === 'post') {
        return http.post(this.action, model, true, responseType)
            .then(this.exportResponseHandler)
            .catch(err => console.error(err));
      }

      return http.get(`${this.action}?${qs.stringify(model)}`, {}, responseType)
            .then(this.exportResponseHandler)
            .catch(err => console.error(err));
    },
