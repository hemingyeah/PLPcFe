...
function getVueInstance(component: Vue | Function | null | undefined | any): Vue {
  ...
  return new Vue({ name: ComponentNameEnum.Empty })
}

/**
 * @description: vue挂载方法
 * @param {Vue | Function} component 组件或者函数
 * @param {string} elementOrSelector 需要挂载的元素或者是选择器
 * @return {Vue} vue实例
*/
export function mount(component: Vue | Function, elementOrSelector: string | Element): Vue | null {
  if (!elementOrSelector) {
    Log.warn('Caused: elementOrSelector is required', mount.name)
    return null
  }
