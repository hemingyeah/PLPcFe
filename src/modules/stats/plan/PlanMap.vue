  ...
  methods: {
    ...
    async renderMap(data = []){
      ...
      data = data.filter(item => {
        ...
        return customerAddress.longitude && customerAddress.latitude;
      });

      data.forEach(item => {
        let customerAddress = item.customerAddress || {};
        let key = generateKey(customerAddress.longitude , customerAddress.latitude);
    ...
    createMarker(data){
      ...
      let marker = new AMap.Marker({
        position: [customerAddress.longitude,customerAddress.latitude],
        extData: data,
      ...
      marker.on('click',event => {
        ...
        let key = generateKey(ca.longitude, ca.latitude);
        this.taskList = this.taskMap[key] || [];
      })
    ...
    fetchData(){
      ...
      return http.get('/stats/plan/unFinishedTask', params)
    },
    openTask(event, task){
