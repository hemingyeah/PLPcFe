<template>
  <div class="stats-task-chart-wrap">
    <div class="stats-task-chart">
      <div id="service-type-chart" ref="chart"></div>
  ...
  methods: {
    ...
    init(){
      ...
      chart.on('click', event => {
        ...
        this.model.serviceType = origin.name;
    ...
    render(){
      ...
      let serviceType = this.chartData.serviceType || [];

      let arr = [];

      for(let name in serviceType){
        if(serviceType[name] && serviceType[name] > 0) arr.push({
          count: serviceType[name],
          name: name
        });
      }
