    ...
    <div class="stats-task-table-wrap" v-if="showTable">
      ...
      <div class="stats-task-table">
        <el-table :data="page.list" class="stats-table">
          ...
          <el-table-column label="产品类型" prop="products">
            <template slot-scope="scope">
              {{scope.row  | prettyProductType}}
            </template>
          </el-table-column>
  ...
  methods: {
    ...
    init(){
      ...
      chart.on('click', event => {
        ...
        this.model.productType = origin.name;
    ...
    render(){
      ...
      let productType = this.chartData.productType || {};
      let arr = [];
  ...
  filters: {
    prettyProductType(value){
      ...
      if(products != null && products.length > 0){
         products.forEach(item => productType.push(item.type));
      }else{
        if(product) productType.push(product.type);
      }
