<template>
  <div class="stats-task-chart-wrap">
    <div class="stats-task-chart">
      <div id="task-type-chart" ref="chart"></div>
...
export default {
  ...
  methods: {
    ...
    exportData(){
      let model = {
        taskType: this.model.templateName,
        tagId: this.teamId
      };
    ...
    init(){
      ...
      chart.on('click', event => {
        ...
        this.model.templateName = origin.taskType;
    ...
    render(){
      ...
      let arr = this.chartData.taskType || {};

      let taskType = [];

      for(let name in arr){
        if(arr[name] && arr[name] > 0) taskType.push({
          taskType: name,
          count: arr[name]
        });
      }
      ...
      let taskTypeOption = this.getOptions(taskTypeLegend, taskTypeSeries, '合计：' + sum)
