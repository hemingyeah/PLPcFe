      ...
      <el-table stripe :data="specificData" class="stats-table">
        <el-table-column
          ...
          <template slot-scope="scope">
            <template v-if="column.field === 'sale'">
...
<script>
  ...
  export default {
    name: 'service-usage-crowd-report',
    data() {
      ...
      return {
        ...
        requestOption:{
          ...
          url: '/stats/servicePrice/getNumberByExecutor',
          exportDataUrl: '/stats/servicePrice/exportListByServicePriceExecutor?',
          specificDataUrl: '/stats/servicePrice/getListByServicePriceExecutor',
        },
        paramsBackup: null,
        btnText: '加载更多数据',
        loading: false,
    ...
    methods: {
      ...
      exportData(type) {
        ...
        if(type === 'export') {
          params.executorId = this.specificDataParams.executorId;
        }
      ...
      // The parent componemt changes search condition(eg: tagId, timeStart, timeEnd) and set pageNum to 1.
      loadChartData(p = {}) {
      ...
      initChart(){
        ...
        const dom = document.getElementById("usage-crowd-chart");
        if (!dom) return;
        ...
        if (!chart._$handlers.click || !chart._$handlers.click.length) {
          chart.on('click', event => {
            ...
            this.specificDataParams.executorId = origin.executorId;
