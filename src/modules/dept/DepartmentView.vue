      ...
      <div v-else class="department-main-right">
          ...
          <div class="department-user-table" v-if="subDepartments.length > 0">
            <el-table class="team-table" :data="subDepartments" stripe header-row-class-name="team-detail-table-header">
              ...
              <el-table-column label="部门位置" show-overflow-tooltip>
                <template slot-scope="scope">
                  {{scope.row.tagAddress | fmt_address}}
                </template>
              </el-table-column>
        ...
        <div class="department-user-block">
          ...
          <div class="department-user-block-header">
            ...
            <base-button type="primary" @event="openCreateUserPanel"> 新建成员账号 </base-button>
          ...
          <div v-else class="no-data-block">
            当前部门暂无人员 <span v-if="allowAddUser" class="active-btn" @click="chooseUser()">添加成员</span>
          </div>
...
export default {
  ...
  computed: {
    ...
    roleDes(){
      if(this.selectedRole.id == 1) return '系统管理员拥有所有权限，不允许删除和编辑！';
      if(this.selectedRole.id >= 2 && this.selectedRole.id <= 10) return '系统内置的角色，支持编辑角色权限和重置权限';
  ...
  methods: {
    ...
    openMap() {
      this.$fast.map.display(this.deptInfo.tagAddress, {title: '部门位置'})
    ...
    async chooseRole(role){
      ...
      return this.getRoleUserList(this.roleParams).then(rolePage => {
    ...
    async delDepartment(id){
      ...
      try {
        if(await this.$platform.confirm('您删除的部门，如果有子部门，也将一并删除，是否继续删除?')){
    ...
    addDeptUser(users){
      ...
      TeamApi.addUser(params).then(result => {
        let isSucc = result.status == 0;
        if(isSucc) {
          // 刷新组织架构 成员列表
          this.initialize(false);
    ...
    addRoleUserList(users){
      ...
      addRoleUser(params).then(result => {
        let isSucc = result.status == 0;
        if(isSucc) {
          // 刷新列表
          this.chooseRole(this.selectedRole);
    ...
    fetchUser(){      
      ...
      return TeamApi.userList(params).then(userPage => {
    ...
    roleUserDelete() {
      ...
      delRoleUser(params).then(result => {
        let isSucc = result.status == 0;
    ...
    userDelete() {
      ...
      TeamApi.deleteUser(params).then(result => {
        let isSucc = result.status == 0;
