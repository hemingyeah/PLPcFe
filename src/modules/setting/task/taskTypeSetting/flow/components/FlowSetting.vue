        ...
        <div class="flow-setting-right">
            <div class="setting-specific">
                ...
                <div class="setting-specific-approve">
                    ...
                    <approve-setting :options="approveOptions" :approveSetting="flowSetting.approveSetting" @change="changeApproveSetting"/>
                </div>
                <!--E 审批设置 -->
                ...
                <div v-if="showOvertime" class="setting-specific-overtime">
                    <h2>
                        超时提醒<el-switch class="ml-12" v-model="flowSetting.overTimeState"/>
                    </h2>
                    <div>
                        {{flowMap[type].name}}后超过
                        <el-input class="w-87" onkeyup="if(isNaN(value))execCommand('undo')" v-model="flowSetting.overTime"/>
                        小时后，标记为超时
                    </div>
                    <div>
            ...
            <div class="setting-common">
                <div class="setting-specific-form">
                    <h2>
                        计划时间提醒
                        <el-switch class="ml-12" v-model="taskTypeConfig.planRemindSetting.state" />
                    </h2>
                    <div class="mb-8">
                        在计划时间
                ...
                <div class="setting-specific-form">
                    ...
                    <approve-setting :options="stableOptions" :approveSetting="taskTypeConfig.cancelApproveSetting" @change="(setting) => changeApproveSetting(setting, 'cancel')"/>
                </div>
            </div>
        </div>
        <!--E 设置项 -->
    ...
    methods: {
        ...
        async getTaskFields(type) {
            if(['create','finish'].includes(type)) {
                try {
                    let fields = await TaskApi.getAllFields({ tableName: this.mode, typeId: this.taskTypeId, isFromSetting: false });
                    this.fields = fields || [];
                } catch (error) {
        ...
        updateOvertimeSetting() {
            let index = this.taskTypeConfig.taskOverTimeModels.findIndex(item => item.overTimeState === this.type);
            if(index > -1) {
                this.$set(this.taskTypeConfig.taskOverTimeModels, index, this.taskOverTimeModel);
            }
        },
        /** 格式化审批人员名称 */
