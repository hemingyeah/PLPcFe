  ...
  <div class="part-batchIn-form">
    <el-table :data="form" empty-text="请点击下方按钮添加要入库的数据">
      ...
      <el-table-column label="仓库">
        <el-select v-model="scope.row.repertory" slot-scope="scope">
      ...
      <el-table-column label="类型">
        <el-select v-model="scope.row.type" slot-scope="scope">
          <el-option v-for="option in sparepartConfig.inStoreType" :key="option" :label="option" :value="option"></el-option>
        </el-select>
      </el-table-column>
      <el-table-column label="数量" width="80px">
        <el-input slot-scope="scope" v-model="scope.row.variation" type="number" step="any" :min="0"></el-input>
      </el-table-column>
    ...
    async pack(){
      ...
      try {
        ...
        form.forEach((item, index) => {
          ...
          if(!item.sparepart || !item.repertory || !item.type || !item.variation || item.variation <= 0 || count != -1){
