  ...
  <div class="part-set-safety-stock-form-container">
    ...
    <div class="function-btn-group-container" v-if="formData.length > 1">
      <div class="function-btn-group">
        ...
        <el-button type="text" size="large" class="no-padding" @click="setUnifiedValue('')">批量清除</el-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      formData: this.initData_(this.formdata)
    };
  },
  props: {
    formdata: Array
      ...
      return (
        <span>
          <el-tooltip
            class="item"
            effect="dark"
            content="清空安全库存值或者点击【清除设置】按钮并点击【确定】保存会移除当前备件设置的安全库存值"
            placement="top-end"
          >
            <span>
              设置安全库存值<i class="el-icon-question"></i>
            </span>
          </el-tooltip>
        </span>
      );
