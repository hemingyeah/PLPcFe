...
export default {
  ...
  methods: {
      ...
      TaskApi.getTaskCountByState(searchModel).then((res) => {
        ...
        this.filterData = {
          ...
          all: this._number(allocated) +
            ...
            this._number(costed),
          unfinished: this._number(created) +
            this._number(refused) +
    ...
    buildExportParams(checkedMap, ids, exportOneRow) {
      ...
      cardFieldChecked = cardFieldChecked.filter(item => {
        return item
      })
      /** ********************* *********************/
      // 工单信息
    ...
    seoSet() {
      ...
      this.seoSetList = [...taskFields.filter(item => {
        return item.isSystem === 1 && item.displayName !== '工单编号' && item.formType !== 'attachment'
      }).map(item => {
        ...
        return item
      }), ...linkman_list, ...address_list, ...product_list, ...Inquire]
    },
    /**
     * @description 初始化page
     */
    ...
    search(searchModel = '') {
      ...
      switch (selectId) {
        ...
        case 'create':
          mySearch = {
            createUser: initData.currentUserId
          };
          this.searchParams.executor = '';
          this.searchParams.synergyId = '';
          break;
        case 'execute':
          mySearch = {
            executor: initData.currentUserId
          };
          this.searchParams.createUser = '';
          this.searchParams.synergyId = '';
          break;
        default:
          mySearch = {
            synergyId: initData.currentUserId
          };
          this.searchParams.createUser = '';
          this.searchParams.executor = '';
          break;
      ...
      if (!searchModel) {
        ...
        const par = {
          ...
          searchTagIds: params.tags && params.tags.map(({
            id
          }) => id),
          systemConditions
        };
