  ...
  <base-modal title="工单用时" :show.sync="visible" width="500px" class="task-time-dialog">
    <div class="base-modal-content">
      <div class="time-axis-header">
        ...
        <div class="time-axis-item">
          <el-tooltip effect="dark" content="从创建到开始用时" placement="left">
            <label>响应用时：</label>
          </el-tooltip>
          <span>{{receiptTaskForUpdate.taskResponseTimeStr | filterTimeSpent}}</span>
        </div>
        <div class="time-axis-item">
          <el-tooltip effect="dark" content="从创建到完成用时" placement="left">
            <label>完工用时：</label>
          </el-tooltip>
          <span>{{receiptTaskForUpdate.createToCompleteUsedTimeStr | filterTimeSpent}}</span>
        </div>
      </div>
    </div>
  </base-modal>
