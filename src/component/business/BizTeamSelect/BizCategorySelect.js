...
const BizTeamSelect = {
  ...
  props: {
    ...
    fetchFunc: {
      type: Function,
      default(params){ 
        return CallCenterApi.getZxSortList(params)
      }
    },
    serializer: {
      type: Function,
  ...
  methods: {
    ...
    renderItem(h, item){
      ...
      return (
        <div class={clazz} onClick={e => this.choose(item)} key={item.id}>
          ...
          <div class="biz-team-select-name">{ item.name}</div>
        </div>
      )
    },
    ...
    renderTree(h, items = []){
      if(!this.loading && items.length == 0){
        return <div class="biz-team-select-empty">暂无分类</div>
      }
    ...
    renderTag(item){
      return (
        <div class="biz-team-select-tag" key={item.id}>
          <span class="biz-team-select-tag-text">{item.name || item.name}</span>
