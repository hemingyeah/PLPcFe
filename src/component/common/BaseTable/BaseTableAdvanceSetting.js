...
const BaseTableAdvancedSetting = {
  ...
  methods: {
    ...
    /* 是否是禁用状态 */
    isDisabled(row = {}) {
      return row.show === 'important' || row.disabled === true
    },
    /** 显示设置窗 */
    open(columns) {
              ...
              return (
                <div class={['base-table__setting-row', row.show ? null : 'base-table__setting-row-disable']} >
                  ...
                  <div class="base-table__setting-row-name">
                    <span onClick={e => this.handlerRowNameClicked(row)} style={{ width: `${row.width}px` }} key={row.width}>{row.label}</span>
                  </div>
