<template>
  <base-modal 
    ...
    <div class="bm-picker-map" ref="container"></div>
    <div class="bm-picker-list" v-if="addressInfo.show">
      <h3>当前选择位置</h3>
      ...
      <template v-if="addressInfo.pois.length > 0">
        <h3>附近地点</h3>
        <div class="bm-address-item" v-for="item in addressInfo.pois" :key="item.id" :class="{'bm-address-checked': item.checked}">
      ...
      <div class="bm-window-body">
        ...
        <template v-else>
          ...
          <button 
            ...
            v-if="chosenItem && chosenItem.formattedAddress != '无效地址，请重新选择'">使用地址</button>
        </template>
      </div>
    </div>
