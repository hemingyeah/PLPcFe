...
router.get('/documet/list', async ctx => {
  let script = ['/document.list.view.js'];
  let modConfig = modules['document.list'];
  let reqHeaders = ctx.request.headers;
  let result = await HttpClient.request('/documet/list', 'get', null, {headers: reqHeaders});
...
router.get('/documet/create', async ctx => {
  let script = ['/document.create.view.js'];
